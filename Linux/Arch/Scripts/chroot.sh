...
options cryptdedvice=UUID=$luksuuid:luks:allow-discards root=/dev/mapper/luks rootflags=subvol=@ rd.luks.options=discard nvidia_drm.modeset=1 amd_iommu=on rw" >> /boot/loader/entries/arch.conf
  echo "title Arch Linux (Zen)
linux /vmlinuz-linux-zen
initrd /amd-ucode.img
initrd /initramfs-linux-zen.img
options cryptdedvice=UUID=$luksuuid:luks:allow-discards root=/dev/mapper/luks rootflags=subvol=@ rd.luks.options=discard nvidia_drm.modeset=1 amd_iommu=on rw" >> /boot/loader/entries/arch-zen.conf
}
